% Load images
disp5to1 = double(imread('DisparityBasic5to1_w3.png'));
disp1to5 = double(imread('DisparityBasic1to5_w3.png'));
...
for base_pixel_y = 1: image_height_original
  for base_pixel_x = 1: image_width_original
    if isnan(consistency_matrix1(base_pixel_y, base_pixel_x))
    else  
    ...
    if isnan(consistency_matrix2(base_pixel_y, base_pixel_x))
    else
...
imwrite(consistency_matrix1, newmap, 'ConsistencyCheckResult1_w3.png');
imwrite(consistency_matrix2, newmap, 'ConsistencyCheckResult2_w3.png');